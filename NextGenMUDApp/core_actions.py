...
class CoreActions:
    ...
    async def do_aggro(cls, actor: Actor):
        ...
        if actor.temporary_character_flags_.are_flags_set(CharacterFlags.IS_SITTING):
            ...
            return
        elif actor.temporary_character_flags_.are_flags_set(CharacterFlags.IS_SLEEPING):
            msg = "You can't aggro while sleeping!"
            await actor.echo(CommTypes.DYNAMIC, msg, set_vars(actor, actor, actor, msg), game_state=cls.game_state_)
            return
        elif actor.temporary_character_flags_.are_flags_set(CharacterFlags.IS_STUNNED):
        ...
        for c in actor.location_room_.characters_:
            if c != actor and c.actor_type_ == ActorType.CHARACTER:
                msg = "You attack {article_plus_name(c.article_, c.name_)}!"
                set_vars(actor, actor, c, msg)
                actor.echo(CommTypes.DYNAMIC, msg, vars, game_state=cls.game_state_)
                msg = "{article_plus_name(actor.article_, actor.name_, cap=True)} attacks you!"
                set_vars(actor, actor, c, msg)
                c.echo(CommTypes.DYNAMIC, msg, vars, game_state=cls.game_state_)
                msg = "{article_plus_name(actor.article_, actor.name_, cap=True)} attacks {article_plus_name(c.article_, c.name_)}!"
                set_vars(actor, actor, c, msg)
