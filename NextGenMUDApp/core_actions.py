...
class CoreActions(CoreActionsInterface):
    ...
    async def arrive_room(cls, actor: Actor, room: Room, room_from: Room = None):
        ...
        await cls.do_look_room(actor, actor.get_room())
        ...
        # actor.get_room().send_text("dynamic", f"{actor.name} arrives.", exceptions=[actor])
    ...
    async def world_move(cls, actor: Actor, direction: str):
        ...
        if not direction in actor.get_room().exits:
            raise Exception(f"Location {actor.get_room().id} does not have an exit in direction {direction}")
        
        actor.get_room().echo("dynamic", f"{firstcap(actor.name)} leaves {direction}", exceptions=[actor], game_state=cls.game_state)
        await actor.send_text("dynamic", f"You leave {direction}")
        old_room = actor.get_room()
        destination = actor.get_room().exits[direction]
        if "." in destination:
        ...
        actor.get_room().remove_character(actor)
        actor.set_room(None)
    ...
    async def start_fighting(cls, subject: Actor, target: Actor):
        ...
        await subject.get_room().echo(CommTypes.DYNAMIC, msg,
                                        set_vars(subject.get_room(), subject, target, msg),
                                        exceptions=[subject, target], game_state=cls.game_state)
        cls.game_state.add_character_fighting(subject)
        for c in target.get_room().get_characters():
            if c.has_perm_flags(PermanentCharacterFlags.IS_AGGRESSIVE) \
                ...
                set_vars(c, c, target, msg)
                target.get_room().echo(CommTypes.DYNAMIC, msg, vars, exceptions=[c], game_state=cls.game_state)
    ...
    async def do_die(cls, dying_actor: Actor, killer: Actor = None, other_killer: str = None):
        ...
        if killer:
            ...
            await dying_actor.get_room().echo(CommTypes.DYNAMIC, msg,
    ...
    async def process_fighting(cls):
        ...
        for c in cls.game_state.get_characters_fighting():
            ...
            if total_dmg > 0 and c.fighting_whom != None:
                ...
                await c.echo(CommTypes.DYNAMIC, msg, set_vars(c, c, c.fighting_whom, msg), game_state=cls.game_state)
                msg = f"You are {status_desc}"
                await c.fighting_whom.echo(CommTypes.DYNAMIC, msg, set_vars(c.fighting_whom, c, c.fighting_whom, msg), game_state=cls.game_state)
                msg = f"{c.fighting_whom.art_name_cap} is {status_desc}"
                await c.get_room().echo(CommTypes.DYNAMIC, msg,
                                            set_vars(c.get_room(), c, c.fighting_whom, msg),
                                            exceptions=[c, c.fighting_whom], game_state=cls.game_state)
    ...
    async def do_aggro(cls, actor: Actor):
        ...
        if actor.has_temp_flags(TemporaryCharacterFlags.IS_SITTING):
            ...
            await actor.echo(CommTypes.DYNAMIC, msg, set_vars(actor, actor, actor, msg), game_state=cls.game_state)
            return
        for c in actor.get_room().characters:
            if c != actor and c.actor_type == ActorType.CHARACTER:
                ...
                set_vars(actor, actor, c, msg)
                actor.get_room().echo(CommTypes.DYNAMIC, msg, vars, exceptions=[actor, c], game_state=cls.game_state)
